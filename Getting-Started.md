...
***
Knowing an action's keyword and parameter order usually comes with practice using HTSL, but while you learn there's a few ways for you to find out.
1. Create the action you want to use ingame, then use the export feature to add it to a file. This will have the correct keyword and you just need to substitute text in for each parameter, preserving the order
2. The parameter order for a lot of actions follow the ingame order in the menu, so you may be able to guess.
3. This wiki will have proper documentation for every action that should show how to write it. _(Until that is written, the [guide for the ChatTriggers version of HTSL](https://hypixel.net/threads/updated-guide-htsl.5555038/) is mostly up-to-date)_